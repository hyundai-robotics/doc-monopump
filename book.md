...
감속기를 사용하지 않고 모터와 직결한다면 감속비를 360:1으로 설정합니다. 이는 모터 1회전에 실제 기구는 360deg 회전함을 의미합니다. 또한 토출이 진행되는 방향을 고려하여 감속비 부호를 설정합니다. <br>

실러 건에 설정된 가속시간과 감속시간이 크면 토출 시작과 종료시 그리고 석백과 리필 동작시 반응이 늦기 때문에 가능한 범위에서 작게 설정해서 사용합니다.
# 2.2 실러건 데이터 설정
