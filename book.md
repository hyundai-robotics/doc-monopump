...
[자동할당] 버튼에 의하여 선택된 용접기 제조사에 따른 신호의 자동 설정이 가능합니다.
![](../../_assets/image9_1.png)

- 통신 상태 : 실러 제어반과의 통신 상태를 확인합니다. 실러 제어반은 해당 신호를 1초마다 on/off 반복하도록 제어하면 됩니다. 로봇 제어기는 해당 신호의 상태가 1초 이상 변경되지 않으면 "E6319 실링 장비 통신이상" 에러를 발생합니다. <br>
- 그 외 신호 : 실러 제어반의 상태를 확인합니다. 로봇 제어기에서 해당 상태를 검지하여 에러나 경고를 발생하고자 할 때에는 사용자 정의 에러/경고 기능을 사용하십시오.
...
- 에러 리셋 : 실러 제어반에 이상이 발생한 경우에 이를 리셋하기 위한 출력입니다. R1(에러 리셋) 동작이나 "에러/경보 신호 클리어" 신호가 입력되면 동작하는데 동작은 1초 동안 on되는 펄스 신호로 출력됩니다.<br>
- 그 외 신호 : 로봇언어에서 신호출력으로 실러 제어반에 상태를 전달할 때 사용하십시오.
...
- flow_amount : 토출 시작부터 종료까지 계측된 토출량입니다. <br>
...
{% hint style="info" %}
- 제어기 전원 투입 후 첫번째 토출종료 명령에서 새로운 로그 파일이 생성되며 그 후 토출종료 명령을 실행할 때 계측된 토출량이 기록됩니다. 
